...
1. **Translation polish** — `strings/weather.yaml` has a first pass per `LANG_*` (SV, NO, NL, IT, ES, DE×2, FR, JA romaji, EN/EN-GB). Native-speaker review especially for decade compounds (`qual` + `decades`), French 70s–90s phrases, and Japanese `…jūdai` bands.
